...
axiosClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (
      ...
      try {
        // TỰ ĐỘNG GỌI API REFRESH: Xin Backend cấp cho Access Token mới
        // (Lúc này Cookie chứa RefreshToken tự động được gửi kèm nhờ 'withCredentials: true')
        ...
        // Cất Access Token mới toanh vào lại két sắt localStorage
        localStorage.setItem('accessToken', res.data.accessToken);
        
        // Đính kèm Token mới vào lại cái Request vừa bị lỗi lúc nãy
        originalRequest.headers.Authorization = `Bearer ${res.data.accessToken}`;
        
        // Gửi lại Request đó lên Backend một lần nữa (Người dùng sẽ không hề nhận ra sự gián đoạn này)
        return axiosClient(originalRequest);
        
      } catch (refreshError) {
        // Nếu xin Token mới mà cũng thất bại nốt (ví dụ: Refresh Token cũng hết hạn luôn)
        // Thì đành phải "đá" người dùng ra ngoài, bắt đăng nhập lại từ đầu.
